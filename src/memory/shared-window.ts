...
export class SharedMemoryWindow implements ISharedMemoryWindow {
  ...
  constructor(config: MemoryWindowConfig, windowId?: bigint) {
    this.config = config;
    this.windowId = windowId ?? (BigInt(Date.now()) << 16n | BigInt(Math.floor(Math.random() * 0xFFFF)));
    ...
    this.metadataView = new Uint8Array(
      this.buffer, 
      HEADER_SIZE + config.windowSize, 
      METADATA_SIZE
    );
  ...
  async acquireWriteLock(timeout: number = 1000): Promise<boolean> {
    ...
    while (performance.now() - startTime < timeout) {
      ...
      if (result === currentFlags) {
        ...
        // Update writer ID
        const writerId = BigInt(process.pid || 0) << 32n | BigInt(Math.floor(Date.now()) & 0xFFFFFFFF);
