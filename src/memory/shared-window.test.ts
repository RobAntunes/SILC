...
import type { MemoryWindowConfig } from '../types/memory.types';
...
if (typeof SharedArrayBuffer === 'undefined') {
  ...
  (global as any).Atomics = {
    store: (arr: any, index: number, value: number) => {
      arr[index] = value;
      return value;
    },
    load: (arr: any, index: number) => arr[index] || 0,
    add: (arr: any, index: number, value: number) => {
      arr[index] = (arr[index] || 0) + value;
      return arr[index];
    },
    sub: (arr: any, index: number, value: number) => {
      arr[index] = (arr[index] || 0) - value;
      return arr[index];
    },
...
describe('SharedMemoryWindow', () => {
  ...
  describe('read operations', () => {
    ...
    test('should allow multiple concurrent readers', async () => {
      ...
      const reads = Promise.all([window.read(), window.read(), window.read()]);
  ...
  describe('concurrent operations', () => {
    test('should handle concurrent writes safely', async () => {
      ...
      const [result1, result2] = await Promise.all([window.write(data1), window.write(data2)]);
