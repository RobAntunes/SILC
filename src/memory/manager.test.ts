...
import type { MemoryPoolConfig } from '../types/memory.types';
...
describe('MemoryManager', () => {
  ...
  describe('concurrent operations', () => {
    test('should handle concurrent allocations', async () => {
      const allocations = Array(10)
        .fill(null)
        .map(() => manager.allocateWindow());
    ...
    test('should handle concurrent pool allocations', async () => {
      ...
      const allocations = Array(10)
        .fill(null)
        .map(() => manager.allocateWindow(512, 'concurrent-pool'));
  ...
  describe('performance', () => {
    test('should allocate windows quickly', async () => {
      ...
      await Promise.all(
        Array(100)
          .fill(null)
          .map(() => manager.allocateWindow(1024)),
      );
